...
import { DEFAULT_FEE_BPS, feeFor as sharedFeeFor } from '@deca/shared';
...
/** Trading fee in cents for a notional in cents, at a basis-point rate (delegates to shared). */
export function feeFor(notionalCents: number, bps: number = DEFAULT_FEE_BPS): number {
  return sharedFeeFor(notionalCents, bps);
}
