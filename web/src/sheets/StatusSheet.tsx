...
export default function StatusSheet({ open, onClose, onClosed }: RoutedSheetProps) {
  ...
  const sessionTip = useInlineTip('session');
    ...
    >
      {!m ? (
        ...
        <div className="bx-sheet-body">
          ...
          <p className="t-subhead bx-status-sheet__session">{m.sessionLine}</p>
          ...
          <InsetGroupedList surface="sheet" aria-label={SHELL.howGameWorks}>
            <KeyValueRow label={GLOSSARY.session?.label ?? 'Session'} info={sessionTip.button} value={`${m.session} / ${m.sessions}`} />
            {sessionTip.panel}
