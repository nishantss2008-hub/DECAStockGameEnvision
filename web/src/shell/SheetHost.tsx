 ...
 * out, so Back and swipe-dismiss both play the exit. `?sheet=help` is rendered by the page that owns the set.
 */
import { lazy, Suspense, useEffect, useState } from 'react';
import { useSheet, type RoutedSheetProps } from './useSheet';
import type { SheetRequest } from './sheetParams';
...
export function SheetHost() {
  ...
  useEffect(() => {
    if (sheet && sheet.kind !== 'help') setShown(sheet);
  }, [sheet]);
